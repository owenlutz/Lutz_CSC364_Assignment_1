...
def create_socket(host, port, message: str):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        try:
            ...
            print(f"[router_3] Connected to server at {host}:{port}")
            client_socket.sendall(message.encode("utf-8"))
        except ConnectionRefusedError:
            print("[router_3] Connection refused. Make sure the next router is running.")
        except Exception as e:
            print(f"[router_3] An error occurred: {e}")
        print("[router_3] Connection closed.")
...
def handle_client(client_socket, client_address, forwarding_array):
    print(f"[router_3] Accepted connection from {client_address}")
    try:
        ...
        if row[3] <= 0:
            if is_final_router:
                print(f"[router_3] Packet {row} delivered at final router (TTL=0)")
                log_line(OUTPUT_TTL0_LOG, ",".join(map(str, row)))
            else:
                print(f"[router_3] Packet {row} dropped due to TTL=0")
        ...
        if is_final_router:
            print(f"[router_3] Packet {row} delivered at final router")
        ...
        print(f"[router_3] Socket error: {e}")
    except Exception as e:
        print(f"[router_3] Error handling client: {e}")
    finally:
        client_socket.close()
...
def router_3_main():
    ...
    print(f"[router_3] server is listening on {server_address}")
