...
LISTEN_PORT = 8004
FORWARDING_TABLE_PATH = "router_4_table.csv"

OUTPUT_DIR = "./output"
RECEIVED_LOG = os.path.join(OUTPUT_DIR, "received_by_router_4.txt")
SENT_LOG = os.path.join(OUTPUT_DIR, "sent_by_router_4.txt")
DISCARDED_LOG = os.path.join(OUTPUT_DIR, "discarded_by_router_4.txt")
OUT_LOG = os.path.join(OUTPUT_DIR, "out_router_4.txt")
...
def create_socket(host, port, message: str):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        try:
            ...
            print(f"[router_4] Connected to server at {host}:{port}")
            client_socket.sendall(message.encode("utf-8"))
        except ConnectionRefusedError:
            print("[router_4] Connection refused. Make sure the next router is running.")
        except Exception as e:
            print(f"[router_4] An error occurred: {e}")
        print("[router_4] Connection closed.")
...
def handle_client(client_socket, client_address, forwarding_array):
    print(f"[router_4] Accepted connection from {client_address}")
    try:
        ...
        if row[3] <= 0:
            if is_final_router:
                print(f"[router_4] Packet {row} delivered at final router (TTL=0)")
                log_line(OUTPUT_TTL0_LOG, ",".join(map(str, row)))
            else:
                print(f"[router_4] Packet {row} dropped due to TTL=0")
        ...
        if is_final_router:
            print(f"[router_4] Packet {row} delivered at final router")
        ...
        print(f"[router_4] Socket error: {e}")
    except Exception as e:
        print(f"[router_4] Error handling client: {e}")
    finally:
        client_socket.close()
...
def router_4_main():
    ...
    print(f"[router_4] server is listening on {server_address}")
